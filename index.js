...
// 1) her seherde ne qeder insan qaldigini tapan function qurun
  ...
  function Population(cityArray) {
    let totalPopulation = 0;
    for (let i = 0; i < cityArray.length; i++) {
      totalPopulation += cityArray[i].population;
    }
    return totalPopulation;
  }
  const totalPopulation = Population(users);
  console.log('Cem İnsan Sayi: ', totalPopulation);  
...
// 2) objectler icinde ne qeder user ,admin ,moderator oldugunu tapin ve array daxilinde loga cixaran func yazin
  ...
  function Roles(userArray) {
    ...
    console.log("User sayisi:", roleCount.user);
    console.log("Admin sayisi:", roleCount.admin);
    console.log("Moderator sayisi:", roleCount.moderator);
  }
  Roles(users);
  
  // 3) "department": "Accounting" olanlari secib  array sekilinde ekrana cixaran func yazin
  
  function accountingDepartment(employeeArray) {
    const accountingEmployees = [];
    employeeArray.forEach(employee => {
      if (employee.department === "Accounting") {
        accountingEmployees.push(employee);
      }
    });
  
    console.log(accountingEmployees);
  }
  
  accountingDepartment(users);
  
  // 4) emailde gmail.com ile bitenleri secib bir arraya yigan func yazin 

  
  function gmailUsers(userArray) {
    const gmailUsers = userArray.filter(user => user.email.endsWith("@gmail.com"));
